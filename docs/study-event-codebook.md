...
| `reanalysis_started` | A passage or character decision began reanalysis, including submissions with no text change. For character decisions, `change_count` counts actual text differences, not the decision itself. | `inconsistency_id`, `inconsistency_type`, `reanalysis_attempt_id`, `source`, `change_count` |
...
| `manual_edit_finished` | A free edit was submitted for review. `text_changed` is false when the submitted text is identical to the snapshot taken before editing. It does not end work timing. | `inconsistency_id`, `paragraph_count`, `change_types`, `removed_texts`, `added_texts`, `text_changed` |
